...
void load_target(IMAGE *img,BPNN *net)
{
  ...
  if (!strcmp(expression, "angry")) {
    net->target[1] = TARGET_ANGRY;  /* it's me, set target to HIGH */
  } 
  if (!strcmp(expression, "happy")) {
	  net->target[1] = TARGET_HAPPY;  /* it's me, set target to HIGH */
  }
  if (!strcmp(expression, "neutral")) {
	  net->target[1] = TARGET_NEUTRAL;  /* it's me, set target to HIGH */
  }
  if (!strcmp(expression, "sad")) {
	  net->target[1] = TARGET_SAD;  /* it's me, set target to HIGH */
  }
}
