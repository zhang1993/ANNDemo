			  ...
			  int list_errors)
{
	...
	if ((net = bpnn_read(netname)) == NULL) {
		if (train_n > 0) {
			...
			*/
			net = bpnn_create(imgsize, /*25: original number 4*/ 25, 4);
		} else {
	...
	for (epoch = 1; epoch <= epochs; epoch++) {
		...
		for (i = 0; i < train_n; i++) {
			...
			bpnn_train(net, 0.02, 0.6, &out_err, &hid_err);
...
int evaluate_performance(BPNN *net,double *err)
{
	...
	*err = (0.5 * delta * delta);
	...
	double max = 0;
	for (int i = 1; i <= 4; i++){
		if (net->output_units[i] > max){
			max = net->output_units[i];
			maxnum = i;
		}
	}
	if (net->target[maxnum] == 1)
		return 1;
	else
		return 0;
